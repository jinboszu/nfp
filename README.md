...
`nfp.nfp(P, Q)` computes the NFP of the sliding polygon `Q` relative to the fixed polygon `P`. Both input polygons must be convex and represented as counterclockwise lists of `(x, y)` tuples. The resulting NFP is returned in the same format.
